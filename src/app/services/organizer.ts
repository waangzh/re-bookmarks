...
function isUnclassifiedFolderName(name?: string) {
  return Boolean(name && LEGACY_UNCLASSIFIED_FOLDER_NAMES.includes(name.trim()));
}
...
function buildUnclassifiedPath(
  ...
  if (settings.unclassifiedHandling !== "preserveSourcePath") {
    return [UNCLASSIFIED_FOLDER_NAME];
  }

  const sourcePath = normalizeFolderPath(bookmark.path, Math.max(1, bookmark.path.length));
  if (!sourcePath.length) return [UNCLASSIFIED_FOLDER_NAME];
...
function buildMovePlan(
  ...
  const shouldUseUnclassifiedPath = isUnclassifiedResult(classification);
  const path = shouldUseUnclassifiedPath
    ? buildUnclassifiedPath(bookmark, settings)
    : normalizeCategoryPath(
...
function compactClassificationResults(
  ...
  const validResults = [...results.values()].filter((result) => {
    ...
    return first && !isUnclassifiedFolderName(first);
  });
...
function extractCategoryScheme(results: Map<string, ClassificationResult>) {
  ...
  for (const result of results.values()) {
    const first = result.categoryPath?.[0];
    if (first && !isUnclassifiedFolderName(first)) {
...
function uniqueFolderCount(plans: MovePlan[]) {
  return new Set(
    plans.map((plan) => normalizeFolderPath(plan.toFolderPath, Math.max(1, plan.toFolderPath.length)).join("/"))
  ).size;
}
...
export async function undoLastOrganize(): Promise<OrganizeReport | null> {
  ...
  for (const plan of movePlan) {
    try {
      ...
      const currentBookmark = await getBookmark(plan.bookmarkId);
