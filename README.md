...
[版本亮点](#-v170-版本亮点) · [核心能力](#-核心能力) · [偏好学习](#分类偏好无感学习) · [安装](#-安装与体验) · [使用流程](#-使用流程) · [隐私与权限](#-隐私与权限) · [本地开发](#-本地开发) · [路线图](#-路线图)
...
ReMarks 在两者之间提供了一条可控路径：读取浏览器书签树，结合本地规则、已经学到的分类偏好与 OpenAI-compatible AI 服务生成建议；你可以先检查目标文件夹、置信度和原因，手动调整预览结果，确认后才会真正移动书签。每次修正也会成为后续分类的参考，不需要手动维护 Prompt。
...
- **更容易开始**：首次使用引导会在本地扫描书签、连接 AI 并生成小样本，确认分类风格后再进入完整整理。
- **更安心地确认**：整理预览按“自动接受候选、批量确认、强制审核、保持原位”分级，高风险变更必须明确决定。
- **更灵活地调整**：整理前可选择本次模型；待处理推荐支持编辑分类，未分类书签可手动归档或忽略。
- **越用越符合习惯**：修改或拒绝 AI 分类时自动记录偏好，并在下一次分类中优先参考域名、目录、层级和分类风格。
...
- 当用户把 AI 建议的 A 分类改为 B、拖拽到其他目录、手动移动书签或拒绝推荐时，自动记录反馈。
- 学习内容包括“原分类 → 修正分类”、域名常用目录、拒绝的新目录或域名分类，以及一级/二级结构和主题/用途分类倾向。
...
- 根据目标目录、置信度和现有分类习惯进行风险分级：高可信候选可抽查，中等风险按主题批量确认，高风险变更必须逐项决定。
...
1. **完成首次使用引导**：ReMarks 会先在本地扫描书签，再引导你连接 AI、测试配置并生成一组只预览不移动的小样本。也可以跳过引导，稍后在设置页配置 Provider。

2. **选择书签与整理模式**：从侧边栏进入智能整理，选择书签或文件夹。快速整理优先速度；深度整理会等待更多网页元数据。

3. **选择本次模型**：使用 Provider 返回的模型列表，或手动输入兼容模型。本次选择不会覆盖设置页的默认模型。

4. **检查并调整预览**：抽查高可信候选、批量确认中等风险项，并逐项处理强制审核项；如有需要，可把书签拖到其他预览文件夹。修改或拒绝分类后，ReMarks 会立即在本地记录反馈并显示“已记住”提示。

5. **确认执行**：ReMarks 先保存完整书签备份，再逐条执行移动。单项失败不会中断整批任务。

6. **持续优化建议**：后续分类会读取已学习的修正、域名、目录层级和分类风格偏好，生成更贴近个人习惯的建议。

7. **查看报告或恢复**：在报告中查看移动结果和失败项；可撤销或重新应用最近一次整理，也可从备份页安全恢复。需要时可进入“分类偏好”查看或管理学习结果。

```text
扫描书签 → 本地规则与已学习偏好 → URL 脱敏 / 网页元数据
